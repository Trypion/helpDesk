...
exports.findAll = (req, res) => {
    ...
    Ticket.find({}).then((data) => {
        ...
        res.status(500).send({
            message: err.message || "algum erro ao encontrar todos os tickets."
        });
    });
}
...
exports.delete = (req, res) => {
    ...
    Ticket.findByIdAndRemove(id).then((data) => {
        ...
        res.status(500).send({
            message: err.message || "Não foi possivel deletar o ticket com o id: " + id
        });
    });
}
...
exports.deleteAll = (req, res) => {
    ...
    Ticket.deleteMany({}).then((data) => {
        res.send(data);
    }).catch((err) => {
        res.status(500).send({
            message: err.message || "Algum erro ocorreu ao tentar deletar os tickets"
        })
    });
